...
$EMLibVersion = "EM-v0.5.8"; (* combines 5.7.1 and DetNEM-0.2 *)
...
(* ::Text:: *)
(*Main changes :*)
(*- Addition of Parameter NEM routines*)
(*- Fixing Law versions of GMM - NEM routines. Including Full Covariance Matrix routines*)
(*- Update Law routines to make Tolerance & MaxIterations Options instead of fxn args*)
...
(**)
(*Also: *)
(*0. - GmmNDEM behaves erratically [Fix?]*)
(*	- Diag-\[CapitalSigma] Law (GmmNDEMLaw)\.16 + Full-\[CapitalSigma] Law (GmmNDEMFLaw) versions work though. [recheck?]*)
(*1. - Old GMM-NEM condition left out factor of 2. => Old NEM sets half as small as they should be...*)
(*2. - GmmNDEMFLaw works with full covariance matrices \[CapitalSigma] [Done]*)
(*3. - Law EM routines: \[CapitalSigma]-updates use \[Mu](t) instead of \[Mu](t+1) [Done]*)
(**)
(*Deterministic Perturbation Variation on NEM Algorithm implemented for GMM only for now...*)
...
GmmPNEM::usage = "<<fill this later>>";
GmmPNEMFull::usage = "<<fill this later>>";
...
ToleranceLevel::usage = "Specifies stopping rule tolerance level. Stop if different by <10^(-ToleranceLvel)";
MaxIteration::usage = "Max # of Iterations to run EM for";
...
DistributeDefinitions[pzi,fyi,fyzi,Fy,pzjyi,Lyizi,Qv1,Qv2,Q,QN,PlaceParam,
						...
						advNemCond, 
						multNEM, multNEMInd, multNEMneg
]
DistributeDefinitions[GmmEM,GmmNEM,GmmNECM,GmmNEMInd,
	GmmMultNEM, GmmMultNEMInd,GmmMultNEMneg,
	GmmPNEM
]
End[];
...
Options[GmmNDEMLaw]:={
Init\[Alpha]->ConstantArray[1/3,3], Init\[Mu]->2{{-1,1},{-1,-1},{1,1}},Init\[Sigma]->ConstantArray[1,{3,2}],
ToleranceLevel->3, MaxIteration->\[Infinity],
CoolingSchedule->2,NoiseDistribution->(NormalDistribution[0,#]&),
ClusterCount->3,ParameterLock->0,
LStoppingRule->False
};
...
(* ::Subsubsection::Closed:: *)
(*GMM-NEM Noise Conditioning Functions*)
...
data+n (* returns data + noise !!!*)
];(* This version is a bit faster. But it uses a lot more memory in the interval struct *)


(* *)
ndNemCond[data:{__List},means:{__List},dist_,\[Sigma]_/;\[Sigma]<=0]:=data;
ndNemCond[data:{__List},means:{__List},dist_,\[Sigma]_/;\[Sigma]>0]:=Module[
	{dim = Length@data[[1]],d=ConstantArray[0,Dimensions[data]]},
	Table[
			d[[;;,col]]=advNemCond[data[[;;,col]],means[[;;,col]],dist@\[Sigma]],
			{col,dim}
		];
	d
]
...
(* ::Subsubsection::Closed:: *)
(**Parameter* GMM-NEM Noise Conditioning Functions*)


(* ::Text:: *)
(*adv does batch noise addition one dimension (column at a time).*)
(*nd walks through adv for each dimension.*)


advPNemCond[data_List,means_List,dist_]:=Module[
{valints,nind,n=0*Range@Length@data},
valints = Table[
	IntervalUnion[
		Interval[ {Min[0, Max[ 2(data[[j]]-means)]], 0} ],
		Interval[ {0, Max[0, Min[ 2(data[[j]]-means)]]} ]
	],
	{j,Length@data}
];
...
(* *)
ndPNemCond[data:{__List},means:{__List},dist_,\[Sigma]_/;\[Sigma]<=0]:=data;
ndPNemCond[data:{__List},means:{__List},dist_,\[Sigma]_/;\[Sigma]>0]:=Module[
	{dim = Length@data[[1]],d=ConstantArray[0,Dimensions[data]]},
	Table[
			d[[;;,col]]=advPNemCond[data[[;;,col]],means[[;;,col]],dist@\[Sigma]],
			{col,dim}
		];
	d
]
...
obj[cur\[Theta]_,\[Sigma]_]:=Module[{tmp=cur\[Theta],\[Theta]\[Alpha],\[Theta]m,\[Theta]s,\[Alpha],m,s,pzjsums,smptmp},
	...
	If[$EMDebug,Print[k,Grid[Transpose@cur\[Theta],Frame->All] , "\nLikelihood=", Ly[data, cur\[Theta]] ] ];
			...
			tmp[[3]]=If[plock[[3]],
						init\[Sigma],
						smptmp=data;
						If[And[(k>=3),Not[plock[[2]]]], 
							(* No \[Sigma]-noise on 1st iter. when \[Mu]-upd. is active. 
							Since 1st iter is random init *)
							recalc\[Sigma]=\[Sigma]*((k-1)/(k-2))^cs;
							smptmp = ndNemCond[data,Partition[tmp[[2]],dim],ndist,recalc\[Sigma]];
							If[$EMDebug,Print@{k,recalc\[Sigma]}]
						];
						...
						Flatten@(Diagonal/@(Total@Table[
							(Outer[Times,
									(smptmp[[i]]-cur\[Theta][[2,#]]),
									(smptmp[[i]]-cur\[Theta][[2,#]])]*pzjyi[#,data[[i]],cur\[Theta]]
							)&/@Range[Length@First@cur\[Theta]],
						{i, Length@data}]/pzjsums))
					];
			shape[Flatten@tmp,K,dim]
	];
...
Quiet@FixedPointList[
	Check[ obj[#,\[Sigma]n/(k++)^cs], (Abort[]), 
		{DiagonalMatrix::mindet, MultinormalDistribution::posdefprm,
		General::unfl,Power::infy,\[Infinity]::indet}
		]&,
	{init\[Alpha],init\[Mu],init\[Sigma]},
	maxIter,
	SameTest->srule
...
(* ::Subsubsection::Closed:: *)
(*N-D *Parameter* NEM  by "Learning Law"; Diagonal Covariance*)


GmmPNEM[data_List,\[Sigma]n_?NonNegative,opts___?OptionQ]:=
Module[
{obj,init\[Alpha],init\[Mu],init\[Sigma],K,
dim=Length[data[[1]]],cs,ndist,
k=1,plock, recalc\[Sigma]=\[Sigma]n,srule,tol,maxIter},

{init\[Alpha],init\[Mu],init\[Sigma],K,cs,ndist}={Init\[Alpha],Init\[Mu],Init\[Sigma],
	ClusterCount,CoolingSchedule,NoiseDistribution}/.{opts}/.Options[GmmPNEM];

{tol,maxIter}={ToleranceLevel,MaxIteration}/.{opts}/.Options[GmmPNEM];

{plock}={ParameterLock}/.{opts}/.Options[GmmPNEM];

plock = (#==1)&/@IntegerDigits[plock,2,3]; (* Mask-style parameter lock {a,m,s} *)
If[And@@plock, Return[{init\[Alpha],init\[Mu],init\[Sigma]}]];  (* Nothing to update *)
If[Length[init\[Alpha]]<K, init\[Alpha] = ConstantArray[1,K]/K];

obj[cur\[Theta]_,\[Sigma]_]:=Module[
	{tmp=cur\[Theta],\[Theta]\[Alpha],\[Theta]m,\[Theta]s,\[Alpha],m,s,pzjsums,smptmp,zjs=Range[Length@First@cur\[Theta]]},

	If[$EMDebug,
		Print[k,Grid[Transpose@cur\[Theta],Frame->All] , 
		"\nLikelihood=", Ly[data, cur\[Theta]] ] 
		];
			...
			tmp[[3]]=If[plock[[3]],
						...
						If[And[(k>=3),Not[plock[[2]]]], 
							(* No \[Sigma]-noise on 1st iter. when \[Mu]-upd. is active. 
							Since 1st iter is random init and \[Sigma]-NEM depends on knowing Subscript[\[Mu], j]s *)
							recalc\[Sigma]=\[Sigma]*((k-1)/(k-2))^cs;
							smptmp = ndPNemCond[data,Partition[tmp[[2]],dim],ndist,recalc\[Sigma]];
							If[$EMDebug,Print@{k,recalc\[Sigma]}]
						];
						If[plock[[2]], smptmp = ndPNemCond[data,init\[Mu],ndist,\[Sigma]]];
...
If[$EMDebug,
	Print["Starting Law FP iters.\nCluster Count = ", K, "\nInitial Parameters: " 
	Grid[Transpose@{init\[Alpha],init\[Mu],init\[Sigma]},Frame->All]]
];

srule = If[Not[(LStoppingRule/.{opts}/.Options[GmmPNEM])],
			((Max[Abs[Flatten@#1-Flatten@#2]]<10^-tol)&),
			((Abs[Ly[data,#1]-Ly[data,#2]]<10^-tol)&)
];
...
GmmNDEMFLaw[data_List,\[Sigma]n_?NonNegative,opts___?OptionQ]:=
Module[
{obj,init\[Alpha],init\[Mu],init\[Sigma],K,
dim=Length[data[[1]]],cs,ndist,
k=1,plock,recalc\[Sigma]=\[Sigma]n,srule,white\[Sigma],tol,maxIter},
...
{tol,maxIter}={ToleranceLevel,MaxIteration}/.{opts}/.Options[GmmNDEMFLaw];
...
Check[ obj[#,\[Sigma]n/(k++)^cs], (Abort[]), 
	{General::unfl, Power::infy,\[Infinity]::indet}
	(*MultinormalDistribution ::posdefprm,DiagonalMatrix::mindet,*)
]&,
{init\[Alpha],init\[Mu],init\[Sigma]},
maxIter,
SameTest->srule
...
(* ::Subsubsection:: *)
(*N-dim *Parameter* GMM-NEM  by "Learning Law"; Full Covariance*)


GmmPNEMFull[data_List,\[Sigma]n_?NonNegative,opts___?OptionQ]:=
Module[
{obj,init\[Alpha],init\[Mu],init\[Sigma],K,
dim=Length[data[[1]]],cs,ndist,k=1,plock,
recalc\[Sigma]=\[Sigma]n,srule,white\[Sigma],tol,maxIter},

{init\[Alpha],init\[Mu],init\[Sigma],K,cs,ndist,white\[Sigma]}={
	Init\[Alpha],Init\[Mu],Init\[Sigma],ClusterCount,CoolingSchedule,NoiseDistribution,ForceDiagonal
}/.{opts}/.Options[GmmPNEMFull];

{tol,maxIter}={ToleranceLevel,MaxIteration}/.{opts}/.Options[GmmPNEMFull];

{plock}={ParameterLock}/.{opts}/.Options[GmmPNEMFull];
plock = (#==1)&/@IntegerDigits[plock,2,3]; (* Mask-style parameter lock {a,m,s} *)
If[And@@plock, Return[{init\[Alpha],init\[Mu],init\[Sigma]}]];  (* Nothing to update *)
...
srule = If[Not[(LStoppingRule/.{opts}/.Options[GmmPNEMFull])],
			((Max[Abs[Flatten@#1-Flatten@#2]]<10^-tol)&),
			((Abs[Ly[data,#1]-Ly[data,#2]]<10^-tol)&)
];
